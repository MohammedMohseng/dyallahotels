...
import { Plus, Search, ArrowLeft, Edit, User, Phone, Globe, CreditCard } from 'lucide-react'
import { getGuests, getGuestById, createGuest, updateGuest, getCompanies } from '@/actions'
import type { Guest, Stay, Payment, Company, PaymentStatus, StayStatus, RoomType } from '@prisma/client'
...
const stayStatusVariant: Record<StayStatus, 'default' | 'secondary' | 'destructive' | 'outline'> = {
  ACTIVE: 'default',
  COMPLETED: 'secondary',
    ...
    try {
      ...
      const enriched = await Promise.all(
        data.map(async (g: Guest & { company?: { name: string } | null }) => {
          ...
          const activeStay = fullGuest?.stays?.find((s: Stay) => s.status === 'ACTIVE')
          return {
            ...g,
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleCreate} disabled={submitting || isPending}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell">الجنسية</TableHead>
                ...
                <TableBody>
                  {guests.map((guest) => (
                    <TableRow
                      ...
                      onClick={() => onNavigate('guests', { guestId: guest.id })}
                    >
...
function GuestProfileView({
  ...
  const allPayments = (guest.stays ?? []).flatMap(
    (stay: Stay & { payments: Payment[]; room: { roomNumber: string; type: RoomType } }) =>
                      ...
                      {(guest.stays ?? []).map(
                        (stay: Stay & {
                          ...
                          <TableRow key={stay.id}>
                            ...
                            <TableCell>
                              <Badge variant={stayStatusVariant[stay.status as StayStatus]}>
                                {getStayStatusLabel(stay.status)}
                              </Badge>
                            </TableCell>
                            <TableCell>
                              <Badge variant={paymentStatusVariant[stay.paymentStatus as PaymentStatus]}>
              ...
              {allPayments.length === 0 ? (
                ...
                <div className="rounded-md overflow-hidden max-h-96 overflow-y-auto">
                  <Table>
                    ...
                    <TableBody>
                      {allPayments.map((payment: Payment & { roomNumber: string; stayCheckIn: Date }) => (
...
export default function GuestsView({ guestId, search, onNavigate }: GuestsViewProps) {
  if (guestId) {
