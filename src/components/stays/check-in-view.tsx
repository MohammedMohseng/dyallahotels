...
export default function CheckInView({
  ...
  const fetchStays = useCallback(async () => {
    ...
    try {
      ...
      toast.error("فشل تحميل الإقامات");
    }
    setLoading(false);
  }, []);
  ...
  const confirmCheckIn = async () => {
    ...
    try {
      ...
      if (selectedGuest.isNew) {
        toast.error("يرجى إنشاء الضيف أولاً من صفحة الضيوف");
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await performCheckOut(
        checkoutStay.id,
        extraCharges,
        { amount: checkoutAmount, paymentMethod: checkoutMethod },
        userId,
      );
      if (res.error) {
        toast.error(res.error);
  ...
  const nights =
    calcTotal() > 0
      ? nightsBetween(new Date(checkInDate), new Date(expectedCheckout))
      : 0;
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full max-w-md grid-cols-2">
          <TabsTrigger value="stays" className="gap-2">
            <CalendarCheck className="w-4 h-4" /> الإقامات النشطة
          </TabsTrigger>
          <TabsTrigger value="wizard" className="gap-2">
            <Plus className="w-4 h-4" /> تسجيل دخول جديد
          </TabsTrigger>
        </TabsList>
                    ...
                    <TableBody>
                      {stays.map((stay: any) => {
                        const remainingNights = Math.max(
                          ...
                          nightsBetween(
                            new Date(),
                            new Date(stay.expectedCheckOut),
                          ),
                        );
                        return (
                          <TableRow key={stay.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  PAYMENT_STATUS_VARIANT[stay.paymentStatus] ||
                                  "secondary"
                                }
                              >
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setCheckoutAmount(
                                      Math.max(
                                        0,
                                        stay.totalPrice - stay.amountPaid,
                                      ),
                                    );
                                  }}
                                >
        ...
        <TabsContent value="wizard">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                اتبع الخطوات لتسجيل دخول ضيف جديد
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Progress Steps */}
              <div className="flex items-center justify-center gap-2 mb-8">
                {["الضيف", "الغرفة", "تفاصيل الإقامة", "الدفع"].map(
                  (label, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      {i < 3 && (
                        <ArrowRight
                          className={`w-4 h-4 ${i < wizardStep ? "text-primary" : "text-muted"}`}
                        />
                      )}
                    </div>
                  ),
                  ...
                  {!selectedGuest && guestResults.length > 0 && (
                    <div className="border-borderrounded-lg max-h-64 overflow-y-auto">
                      {guestResults.map((g: any) => (
                        <button
                          key={g.id}
                          className="w-full text-right p-3 hover:bg-accent border-b last:border-0 transition-colors"
                          onClick={() => selectGuest(g)}
                        >
                          <p className="font-medium">{g.fullName}</p>
                          <p className="text-sm text-muted-foreground">
                            {g.phone} {g.nationality && `• ${g.nationality}`}
                          </p>
                        </button>
                      ))}
                    </div>
                  ...
                  {showNewGuest && (
                    <div className="border-borderrounded-lg p-4 space-y-3 bg-muted/30">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={confirmNewGuest}>
                    ...
                    {availableRooms.length === 0 ? (
                      ...
                      availableRooms.map((room: any) => (
                        ...
                        >
                          ...
                          <p className="text-sm text-muted-foreground">
                            {ROOM_TYPE_AR[room.type] || room.type} • الطابق{" "}
                            {room.floor}
                          </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        value={numGuests}
                        onChange={(e) =>
                          setNumGuests(parseInt(e.target.value) || 1)
                        }
                      />
                    </div>
                    <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg space-y-3 max-w-lg">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{nights} ليلة</span>
                    </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>الضيف:</span>
                  <span className="font-medium">
                    {checkoutStay.guest.fullName}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>الغرفة:</span>
                  <span className="font-medium">
                    {checkoutStay.room.roomNumber}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>الإجمالي:</span>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setCheckoutStay(null)}>
                  إلغاء
                </Button>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  min={extendStay.expectedCheckOut.split("T")[0]}
                />
              </div>
              {newCheckoutDate &&
                  ...
                  <p className="text-sm text-primary">
                    ...
                    {formatCurrency(
                      nightsBetween(
                        new Date(extendStay.expectedCheckOut),
                        new Date(newCheckoutDate),
                      ) * (extendStay.room?.pricePerNight || 0),
                    )}
                    )
                  </p>
                )}
              <DialogFooter>
                <Button variant="outline" onClick={() => setExtendStay(null)}>
                  إلغاء
                </Button>
